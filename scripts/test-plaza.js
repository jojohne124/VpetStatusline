...
// ── 2z. 長時間開機不可以定格 ───────────────────────
// 回報過「營地的腳色不會移動了」。真因是 MAX_REPLAY 本來寫成
...
// ── 2a. 貼牆的時候不可以在原地抖 ─────────────────────────────────────
// 回報過「營地滿有機會卡在邊界」。真因是「幾乎與牆平行、但帶一點朝外分量」的方向
// 會被截成 1~2 拍，角色每一兩拍就重抽方向，看起來像在原地抖。
// 營地場地小（可走範圍只有 37x25），這件事特別明顯。
{
    for (const [nm, F] of [['廣場', W.PLAZA_FIELD], ['營地', W.YARD_FIELD]]) {
        ...
        // 集中度。門檻是量出來的，不是憑感覺挑：
        //   一開始 廣場 4.8 / 營地 4.3 倍
        //   → 加了 MIN_LEG（貼牆不再原地抖）廣場 4.0 / 營地 3.3
        //   → 加了「撞牆一定往離開牆的方向走」廣場 2.6 / 營地 2.0
...
// ── 7. 名牌不穿透 ────────────────────────────────────────────────────
// ── 營地摸摸的反應幀 ───────────────────────────────────────────────
// 營地的摸摸是**純表演**：只換一幀表情，不動心情值、不寫 ranch.json。
...
console.log('— 顯示名稱 —');
{
    ...
    // 卡片、tree、營地右鍵、statusline 清單全都走 getDisplayName，所以只改那一個地方。
    //
