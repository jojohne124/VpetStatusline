...
const { INSTALL_ROOT, STATE_DIR, ANCHOR_GAP, loadState, saveState, decideAgumon, buildStatusLines, composeOutput, visLen } = require('./agumon-core');

const ART_FILE   = path.join(INSTALL_ROOT, 'agumon-art.json');
...
function flipChar(c) {
    ...
    if (cp >= 0x1FB00 && cp <= 0x1FB3B) {
        return sextantValueToChar(flipSextantValue(sextantCharToValue(cp)));
    }
    if (QUADRANT_REV_MAP.has(c)) {
        return QUADRANT_CHARS[QUADRANT_FLIP_IDX[QUADRANT_REV_MAP.get(c)]];
    }
    return c;
}
...
process.stdin.on('end', () => {
    try {
        ...
        process.stdout.write('statusline-agumon: ' + e.message);
    }
});
